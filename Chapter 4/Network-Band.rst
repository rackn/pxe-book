...
Provisioning can be very network-intensive because it involves dragging over entire operating images from one system to another. It can be even worse if a large amount of canned images is being dragged over. Exceeding the network's bandwidth creates what are called network or broadcast storms, where a large enough portion of the network's resources are consumed by the storm that the network cannot handle normal traffic.
...
The way around the issues with PXE/TFTP is a two-stage install. The two-stage install involves a minimal TFTP component, with the majority of the downloading done using an HTTP connection. It is Digital Rebar's default discovery. Note that this type of install does require some specialization. 
